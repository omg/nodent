...
function contains(ast, fn, descendOn) {
    ...
    if (typeof descendOn !== "function") {
        if (descendOn) {
            ...
            subScopes = function(n) { return !examine(n).isFunction } ;
        }
    }
...
function containsAwaitInBlock(ast) {
    return contains(ast, function(n){
        ...
        var x = examine(n) ;
        return !x.isBlockStatement && !x.isFunction
    });
}
...
function asynchronize(pr, __sourceMapping, opts, logger) {
   ...
   */
    ...
    function mapBlock(node, path, down) {
        var ref = path[0];
        if (examine(ref.self).isBlockStatement && examine(ref.parent).isBlockStatement) {
            var blockScoped = containsBlockScopedDeclarations(node.body) ;
            if (blockScoped && containsAwaitInBlock(node)) {
                ...
                if ('index' in ref) {
                    ...
                    if (deferredCode.length) {
                        ...
                        var blockEnd = node.body[node.body.length - 1];
                        if (!(blockEnd.type === 'ReturnStatement')) {
                            node.body.push(cloneNode(call));
                        }
                        afterDirectives(ref.parent[ref.field],[ctn]);
                    }
                } else {
                    throw new Error("Malformed nested blocks") ;
                }
            } else {
                if (!blockScoped)
                    ref.parent[ref.field].splice.apply(ref.parent[ref.field],[ref.index,1].concat(node.body));
            }
        }
    }
                ...
                if (!returner) {
                    if (callback) {
                        returner = {
                            ...
                            arguments: opts.promises
                              ?[{type: 'ThisExpression'}]
                              :[{type: 'ThisExpression'},containingExits.$error]
                        };
                    } else {
                        returner = {
                            type: 'FunctionExpression',
    ...
    function hoistDeclarations(ast) {
        function isFreeVariable(kinds) {
            return function(n, path) {
                if (n.type === 'VariableDeclaration' && path[0].parent.type != 'ForStatement' && kinds.indexOf(n.kind||'var')>=0) {
            ...
            if (examine(node).isBlockStatement) {
                ...
                if (await) {
                    ...
                    if (isScope) {
                        ...
                        // Change all the delcarations into assigments, since consts
